...
# bootstrap-server.sh — Server bootstrap (Ubuntu 24.04 Server)
# Run as normal user after fresh install

echo "=== Server Bootstrap started ($(date)) ==="
...
# 5. Harden firewall (allow SSH only for now)
...
echo "  2. Run: chmod +x 2.install-server.sh && ./2.install-server.sh"
